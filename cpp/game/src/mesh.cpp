...
mesh::mesh(std::vector<tri> new_triList) : triList(new_triList)
{
...
mesh::mesh(
		...
		std::vector<int> new_indicies) : verticies(new_verticies), indicies(new_indicies)
{
...
void mesh::fromTriList()
{
	...
	for (tri _tri : triList)
	{ // foreach tri in triList
		for (point triPoint : _tri.points)
		{ //foreach point in each tri

			found = false;

			for (unsigned long i = 0; i < verticies.size(); i++)
			{ // every point

				if (triPoint == verticies[i])
				{ //current point is in verticies
					found = true;
