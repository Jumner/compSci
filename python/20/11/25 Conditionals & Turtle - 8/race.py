...
howManyTurtles = 10 # Hell yeah we want them turtles
...
for t in turtles: # Set them up
	...
	t.goto(center,center+150) # Start in the center for circles
...
def selectWinner(turts):
	yCoords = list(map(lambda t: t.ycor(), turts)) # At this point I am just flexing
	index = yCoords.index(min(yCoords))
	turts[index].color("#00FF00")
	turts[index].dot(10)

iterations = int(world.numinput("Race Length", "How far should each turtle go?"))
...
selectWinner(turtles)

world.mainloop() # Wait to not lose the beautiful image